...
#define ISNAN(x) (x != x)
#define HASNAN(x) (ISNAN(x[0]) || ISNAN(x[1]) || ISNAN(x[2]))
#define HANDLE_NANS(x) \
    {\
        if HASNAN(x) memset(x, 0, sizeof(dReal) * 3);\
    }
...
#undef ISNAN
    ...
    if ((b->avel[0] != b->avel[0]) || (b->avel[1] != b->avel[1]) || (b->avel[2] != b->avel[2]))
    {
        b->avel[0] = 0.0;
        b->avel[1] = 0.0;
        b->avel[2] = 0.0;
